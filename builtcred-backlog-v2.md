...
#### 📄 Module Description
This module implements the core classification engine that automatically categorizes every customer as either a Project Advocate or Brand Advocate based on their ownership profile. It establishes the database schema, business logic, and API layer that supports advocate type identification throughout the platform. The module serves as the single source of truth for advocate identity and powers all type-dependent features including eligibility validation, dashboard customization, and analytics segmentation. It must handle edge cases like customers owning multiple properties and support future expansion to additional advocate categories.
...
#### 📄 Module Description
The Validation Engine is a critical security and compliance module that enforces business rules for advocate eligibility before allowing referral submissions. It performs real-time validation of advocate type selection against the customer's ownership profile, target project, and developer relationship. This module prevents unauthorized referrals, detects cross-developer attempts, provides auto-correction where possible, and generates detailed audit logs for compliance. It must be highly performant (sub-200ms) as it's invoked on every referral submission and supports override capabilities for administrative interventions.
...
#### 📄 Module Description
This frontend module provides the user interface for advocate type selection during referral submission. It presents a clear, accessible choice between Project and Brand Advocate options with contextual help text, visual differentiation, and mobile-responsive design. The module integrates with the validation engine to provide immediate feedback on eligibility and guides users toward the correct selection. It emphasizes usability and accessibility (WCAG 2.1 AA) to ensure customers of all technical abilities can successfully self-identify their advocate type.
...
#### 📄 Module Description
This module customizes the advocate dashboard experience based on advocate type, implementing role-based data access and feature visibility. Project Advocates see project-specific content including community updates and neighbor referrals, while Brand Advocates access developer-wide information and multi-project referral capabilities. The module enforces strict data privacy boundaries, prevents information leakage between advocate types, and optimizes data fetching with caching strategies. It serves as the primary user-facing differentiation point in the application.
...
#### 📄 Module Description
The Analytics module provides administrative insights into advocate type performance through comprehensive reporting and visualization. It aggregates metrics including advocate counts, referral rates, conversion percentages, and reward distributions segmented by advocate type. This module enables data-driven decision making for program optimization and supports future differentiation strategies. It includes comparative analysis tools, trend visualization, and export capabilities for financial reporting and stakeholder presentations.
...
#### 📄 Module Description
This module enriches sales team communications by automatically generating contextual descriptions of the referrer relationship based on advocate type. It creates human-readable sales context (e.g., "referred by fellow resident" vs. "referred by existing customer") that appears in CRM systems, email notifications, and lead management interfaces. This context helps sales representatives tailor their approach and leverage the referrer relationship effectively, improving conversion rates through personalized engagement.
...
#### 📄 Module Description
This security-critical module prevents referrals across different developers, enforcing business rules that restrict advocates to referring only within their developer's portfolio. It validates developer relationships at the database level, logs all cross-developer attempts for fraud detection, and implements rate limiting to prevent abuse. The module protects developer proprietary data, maintains program integrity, and provides administrative visibility into potential misuse patterns through comprehensive audit logging and alerting mechanisms.
...
#### 📄 Module Description
The Referral Submission Form is the primary user interface for advocates to submit leads, designed for simplicity and conversion optimization. This module handles form rendering, client-side validation, advocate type selection integration, and seamless API communication. It implements progressive disclosure to reduce cognitive load, provides real-time validation feedback, and supports multiple entry points (dashboard, direct link, mobile). The module must balance data collection needs with user experience, ensuring high completion rates while capturing sufficient information for sales qualification.
...
#### 📄 Module Description
This critical backend module orchestrates the complete referral creation workflow including validation, deduplication, database persistence, CRM integration, and notification triggering. It implements ACID transactions to ensure data consistency, manages asynchronous operations (webhooks, emails) without blocking responses, and generates unique referral identifiers. The module serves as the central processing unit for all referral-related business logic, coordinating between validation services, attribution tracking, database operations, and external integrations while maintaining comprehensive audit logs for compliance and debugging.
...
#### 📄 Module Description
The Link Generation module creates unique, trackable referral URLs for each advocate-project combination, enabling attribution tracking and multi-channel sharing. It implements UUID-based link codes for security and uniqueness, tracks click analytics through UTM parameters, and provides easy copy-to-clipboard functionality. The module supports multiple sharing channels (WhatsApp, SMS, email, social) with pre-filled messaging templates and integrates with analytics systems to measure link performance. Links are permanent and backwards-compatible to support long-term sharing strategies.
...
#### 📄 Module Description
This module generates high-resolution QR codes encoding referral links, enabling offline and print-based sharing strategies. It produces optimized PNG files with error correction for reliability, provides download functionality for advocates, and displays preview images in dashboards. The module integrates with CDN storage for efficient delivery and supports batch generation for administrative use cases. QR codes enable physical marketing materials, in-person referrals at community events, and printed collateral, extending referral reach beyond digital channels.
...
#### 📄 Module Description
The Lead Capture Landing Page is the referred prospect's first interaction with the program, designed to convert interest into actionable leads. This module renders dynamic, SEO-optimized pages featuring project information, referrer attribution (first name only for privacy), lead capture forms, and trust signals. It implements progressive disclosure to maximize conversion, mobile-first responsive design, and fast loading times (<2s). The module integrates with analytics for A/B testing, tracks conversion funnels, and supports personalization based on referral source and advocate type.
...
#### 📄 Module Description
The Duplicate Detection module prevents multiple referrals for the same lead within a 90-day window, maintaining data quality and preventing referral conflicts. It implements high-performance database queries with composite indexes, validates against phone number and project combinations, and provides grace periods for re-engagement scenarios. The module supports administrative overrides for legitimate edge cases, logs all duplicate attempts for pattern analysis, and provides clear user feedback when duplicates are detected. This system protects against both accidental duplicates and intentional gaming of the referral system.
...
#### 📄 Module Description
The Attribution module tracks the complete referral journey by recording both the initial introducer (first-touch) and the final influencer (last-touch) before conversion. It maintains a comprehensive attribution history table, enables future multi-touch attribution modeling, and determines reward allocation (currently first-touch for MVP). This module provides critical data for understanding customer acquisition paths, optimizing advocate engagement strategies, and supporting potential future changes to reward distribution models. It must handle complex scenarios including self-referral prevention and time-decay attribution.
...
#### 📄 Module Description
This analytics module captures and analyzes referral sources across all channels (web portal, WhatsApp, SMS, email, QR codes, social media) to identify the most effective distribution methods. It implements UTM parameter parsing, user-agent detection, referrer analysis, and manual channel tagging. The module provides channel-level reporting including conversion rates, time-to-conversion, and advocate preferences. This data enables optimization of communication strategies, identification of high-performing channels per advocate type, and informed budget allocation for channel-specific promotions.
...
#### 📄 Module Description
The Status Management module tracks referrals through their lifecycle from initial submission through final disposition (converted, declined, or lost). It implements a state machine with validated transitions, maintains complete status history for audit trails, and triggers appropriate notifications at each stage. The module supports sub-status fields for granular tracking (e.g., "site visit scheduled" under "contacted"), accepts sales representative notes, and integrates with CRM systems for bidirectional status updates. This provides transparency to advocates and enables sales team collaboration.
...
#### 📄 Module Description
This financially critical module automates reward computation based on plot value, advocate type, and tiered reward structures. It calculates gross rewards, TDS deductions (10%), net payouts, and buyer discounts while maintaining complete audit trails for accounting. The module implements configurable reward rules to support future differentiation between advocate types, validates all financial calculations with precision arithmetic, and generates reward records linked to conversions. It must handle edge cases including partial payments, cancellations, and reward adjustments while ensuring compliance with tax regulations and developer financial policies.
...
#### 📄 Module Description
The Main Dashboard is the advocate's home page, providing at-a-glance visibility into their referral activity, rewards, and eligibility. This module aggregates data from multiple sources to present key metrics (total referrals, conversions, earnings), displays advocate type badge and ownership details, shows eligible projects, and provides quick action buttons for common tasks. It implements intelligent caching for performance, adapts content based on advocate type, and uses data visualization to make complex information accessible. The dashboard balances information density with clarity to serve both casual and power users effectively.
...
#### 📄 Module Description
The Referral List View provides comprehensive access to all advocate-submitted referrals with advanced sorting, filtering, and search capabilities. This module implements a responsive data table (desktop) with card-based layout (mobile), supports multiple filter dimensions (status, date, project), and enables export to CSV for personal record-keeping. It provides visual status indicators, quick actions, and links to detailed views. The module must handle large datasets efficiently with pagination and implement progressive loading to maintain performance as advocates build extensive referral histories over time.
...
#### 📄 Module Description
The Detail View module provides deep visibility into individual referral records, displaying complete lead information, status timeline, estimated rewards, sales representative contact, and next expected actions. It implements a chronological status history with visual timeline, shows all touchpoints and communications, and provides context for advocates to understand where their referral stands in the sales pipeline. The module supports direct links for easy sharing, includes support contact options, and helps advocates answer their referred contacts' questions about process and timing.
...
#### 📄 Module Description
This module streamlines referral link distribution by providing one-click sharing to multiple channels with pre-filled messaging templates. It implements Web Share API for native mobile sharing, generates platform-specific share URLs (WhatsApp deep links, SMS protocols, email mailto links), and tracks sharing activity for analytics. The module reduces friction in the sharing process through copy-to-clipboard functionality, QR code integration, and customizable message templates that maintain brand voice while allowing advocate personalization. It's critical for maximizing referral reach and lowering barriers to program participation.
...
#### 📄 Module Description
The Rewards Dashboard provides complete financial transparency by displaying total earned, paid, and pending rewards with detailed breakdowns and payment timelines. This module calculates real-time reward status, generates downloadable reward statements (PDF), displays TDS information for tax purposes, and tracks payment history with references. It implements clear visualizations of earnings over time, conversion-to-reward tracking, and projected future earnings based on pipeline referrals. The module builds trust through transparency and helps advocates understand the financial value of their participation.
...
#### 📄 Module Description
The Document Repository provides secure access to advocate ownership documents including allotment letters, payment receipts, registry documents, and construction plans. This module implements document categorization, search functionality, PDF preview capabilities, and secure download with signed URLs that expire after one hour. It integrates with cloud storage (S3), enforces strict access controls ensuring advocates only view their own documents, and logs all document access for security auditing. The repository reduces support burden by giving advocates self-service access to important documents they frequently need.
...
#### 📄 Module Description
The Profile Management module enables advocates to view and update their personal information including contact details, communication preferences, and profile pictures. It implements secure verification workflows for sensitive changes (email verification links, phone OTP), password management with strength requirements, and notification preference controls. The module maintains a balance between allowing user control and protecting critical identity fields (name, advocate type) that require administrative approval. It supports self-service updates to reduce support tickets while maintaining data integrity and security.
...
#### 📄 Module Description
The Notifications Center aggregates all in-app notifications in a centralized interface, providing a persistent history of referral updates, reward notifications, and system announcements. This module implements unread tracking with badge counts, mark-as-read functionality, notification filtering by type, and deep linking to related content. It optionally integrates WebSocket for real-time notification delivery, implements local storage for offline access, and maintains a 30-day notification history. The center ensures advocates never miss important updates even if they miss email or SMS notifications.
...
#### 📄 Module Description
The Admin Dashboard Overview provides executive-level visibility into program performance through aggregated metrics, trend visualizations, and comparative analytics. This module consolidates data from across the system to display KPIs including total advocates by type, active referrals, conversion rates, and rewards paid. It implements customizable date ranges, real-time data updates, and drill-down capabilities to underlying data. The dashboard serves as the landing page for administrators and supports decision-making through clear data presentation, alert highlighting for anomalies, and performance trending to identify program health and optimization opportunities.
...
#### 📄 Module Description
The Advocate Management module provides CRUD operations and oversight for all program participants, enabling administrators to search, filter, view, and manage advocate records. It implements advanced filtering by advocate type, project, status, and activity level; supports bulk operations like notification broadcasting and status updates; and provides export capabilities for reporting and analysis. The module displays key advocate metrics (referrals, conversions, rewards) in a sortable table, links to detailed advocate profiles, and enables administrative interventions including status changes, type corrections, and account management.
...
#### 📄 Module Description
The Admin Advocate Detail View provides comprehensive visibility into individual advocate profiles including ownership history, complete referral activity, conversion summary, and reward earnings. This module aggregates data from multiple sources to create a 360-degree view of advocate engagement, displays activity timelines, shows all touchpoints and communications, and provides administrative action buttons for editing profiles, sending notifications, and managing status. It serves as the single source of truth for understanding advocate behavior, troubleshooting issues, and providing personalized support.
...
#### 📄 Module Description
The Referral Management module serves as the operational hub for processing and tracking all referrals across the program. It provides administrators and sales teams with tools to assign referrals, update statuses, add notes, and manage the sales pipeline. This module implements multi-dimensional filtering (advocate type, status, project, date, sales rep), search across multiple fields, bulk operations for efficiency, and validation override capabilities. It integrates with CRM systems for seamless handoff to sales processes and maintains complete audit trails of all status changes and administrative actions for accountability and reporting.
...
#### 📄 Module Description
The Conversion Management module handles the critical transition from referral to sale, capturing booking details and triggering reward calculations. This module implements a comprehensive conversion form capturing plot details, booking amounts, and payment information; validates data against available inventory; automatically calculates rewards using the reward engine; and initiates all downstream processes including notifications, CRM updates, and financial record creation. It provides confirmation workflows to prevent errors, supports bulk conversion processing for efficiency, and maintains complete conversion history for financial auditing and reconciliation with developer sales systems.
...
#### 📄 Module Description
The Reward Management module provides end-to-end tracking and processing of advocate rewards from calculation through payment. It displays all rewards with payment status, enables marking rewards as paid with payment references, supports bulk payment processing for finance efficiency, and generates payment reports and TDS certificates for tax compliance. This financially critical module maintains complete audit trails, implements validation to prevent duplicate payments, integrates with accounting systems, and provides advocates with transparent visibility into their earnings. It must handle complex scenarios including payment adjustments, cancellations, and partial payments.
...
#### 📄 Module Description
The Project Configuration module enables administrators to manage project settings controlling referral acceptance, reward structures, and program parameters. It provides CRUD operations for projects including name, code, developer assignment, status management, and referral toggles. This module implements validation rules preventing configuration errors (e.g., disabling referrals with active pipeline), supports project-specific reward configuration for future flexibility, and manages the relationship between projects and eligible advocates. It serves as the master data management interface for the physical real estate inventory that underpins the referral program.
...
#### 📄 Module Description
The Analytics & Reports module provides deep insights into program performance through specialized reports and data visualizations. It implements multiple report types including advocate performance rankings, referral pipeline analysis, channel effectiveness, conversion funnel visualization, and financial summaries. This module supports custom date ranges, comparative analysis (time periods, advocate types, projects), and export to PDF/CSV for stakeholder distribution. It leverages aggregation queries, caching strategies, and chart libraries to deliver fast, interactive analytics that enable program optimization and ROI measurement.
...
#### 📄 Module Description
The Bulk Import module enables rapid onboarding of existing customers as advocates through CSV file upload and processing. It implements comprehensive validation (format, required fields, data types), duplicate detection, preview functionality before committing, and row-level error reporting. The module automatically classifies advocate types based on project ownership, handles large files through background job processing, provides detailed success/failure reporting, and supports rollback for critical errors. This tool dramatically reduces administrative burden during initial program launch and periodic customer data synchronization from developer databases.
...
#### 📄 Module Description
The Webhook Configuration interface enables administrators to set up and test CRM webhook integrations through a user-friendly form. This module provides input fields for webhook URLs, authentication credentials, custom headers, and retry policies; implements secure credential encryption in the database; offers test connection functionality to validate configuration; and displays webhook activity logs for debugging. It supports multiple authentication methods (API keys, Bearer tokens, Basic Auth, OAuth) and provides toggle controls to enable/disable integration without losing configuration. This is the control plane for all outbound CRM communication.
...
#### 📄 Module Description
The Outbound Webhook service handles reliable delivery of referral data to CRM systems through asynchronous, queue-based processing. This module implements payload formatting with complete referral and advocate context, HTTP delivery with configurable timeouts, exponential backoff retry logic (3 attempts), comprehensive logging of all attempts, and alerting for repeated failures. It processes webhooks asynchronously to avoid blocking referral submission, handles various response codes appropriately, and maintains high availability through queue persistence. This is the data pipeline ensuring sales teams receive leads in their preferred system without manual data entry.
...
#### 📄 Module Description
The Status Update API enables CRM systems to push referral status changes back to BuiltCred, maintaining synchronization across platforms. This module exposes a RESTful endpoint with API key authentication, validates status transition rules, updates referral records, triggers advocate notifications, and logs all updates for audit. It implements rate limiting to prevent abuse, returns meaningful error messages for validation failures, and supports idempotent requests. This bidirectional sync ensures advocates see real-time progress updates even when sales activities occur entirely within the CRM system, maintaining transparency and engagement.
...
#### 📄 Module Description
The Conversion API allows CRM systems to report successful bookings, triggering automated reward calculation and advocate notifications. This module validates plot availability, processes booking details (plot number, value, date), invokes the reward calculation engine, creates conversion records, updates referral status, and returns calculated reward information in the response. It implements idempotency to prevent duplicate conversions from retry attempts, validates all financial data, and maintains complete audit trails. This API closes the loop on the referral lifecycle by automatically recognizing conversions that occur in external sales systems.
...
#### 📄 Module Description
The Field Mapping module provides flexible configuration to map BuiltCred data fields to CRM-specific field names, accommodating different CRM schemas and custom field structures. It implements a visual mapping interface, stores mapping configurations as JSON, validates required field mappings, supports custom fields, and provides import/export for easy setup replication. The module includes test functionality to preview transformed payloads and ensures data compatibility between systems. This abstraction layer enables BuiltCred to integrate with diverse CRM platforms without code changes, supporting multi-tenant deployments and custom CRM implementations.
...
#### 📄 Module Description
The Integration Health Monitor provides real-time visibility into CRM integration status through metrics dashboards, event logs, and alerting. This module tracks success rates, response times, failure counts, and queue depths for both outbound and inbound integrations; displays recent integration events with status indicators; alerts administrators when thresholds are breached (failure rate >10%, response time >5s); and provides manual retry capabilities for failed webhooks. It serves as the operational dashboard for maintaining integration reliability, enabling proactive issue resolution before advocates or sales teams experience disruption.
...
#### 📄 Module Description
The Email Notification System provides comprehensive email communication capabilities using professional email service providers (SendGrid, AWS SES). This module implements templated email generation with personalization, queue-based sending for reliability, delivery tracking (sent, delivered, opened, clicked), retry logic for failures, and unsubscribe functionality for compliance. It maintains detailed email logs for debugging and analytics, supports HTML templates with responsive design, and handles bulk sending for announcements. The system ensures advocates receive detailed, formatted communications about referral updates, reward calculations, and program information with professional branding.
...
#### 📄 Module Description
The SMS Notification System delivers time-sensitive alerts through SMS messages using Twilio or AWS SNS. This module implements character-limited (160 chars) messaging with template variables, India phone number support (+91 prefix), delivery status tracking, cost monitoring per message, and retry logic (2 attempts). It prioritizes critical notifications (OTP verification, conversions, payments) while managing costs through intelligent message selection. The system provides instant delivery for high-priority events where email latency is unacceptable, ensuring advocates receive immediate confirmation of important actions and outcomes.
...
#### 📄 Module Description
The WhatsApp Notification module leverages WhatsApp Business API to deliver rich, engaging notifications through users' preferred messaging platform. This module implements WhatsApp-approved message templates with rich media support (images, buttons), manages opt-in/opt-out compliance, respects 24-hour conversation window policies, and tracks delivery status. It provides higher engagement rates than email/SMS through familiar interface and supports interactive elements like quick replies and CTAs. The module requires template pre-approval from WhatsApp and careful compliance management but offers superior user experience and engagement for advocates comfortable with the platform.
...
#### 📄 Module Description
The In-App Notification System provides persistent notification visibility within the BuiltCred application through a notification bell icon, dropdown list, and notification center. This module implements real-time notification delivery (optionally via WebSocket), unread count badges, mark-as-read functionality, notification persistence (30 days), and deep linking to related content. It categorizes notifications by type with appropriate icons and supports bulk actions (mark all read). The system ensures advocates never miss updates even if they don't check email/SMS, providing a reliable fallback and permanent notification history accessible anytime.
...
#### 📄 Module Description
The Notification Preferences module empowers users to control their notification experience by selecting channels (email, SMS, WhatsApp, in-app) and frequencies (instant, daily digest, weekly digest) for each notification type. This module implements a preference management interface, stores user choices in the database, enforces preferences across all notification systems, and provides sensible defaults for new users. It respects user autonomy while ensuring critical notifications (security, financial) are always delivered. The system reduces notification fatigue, improves user satisfaction, and maintains engagement by delivering communications in user-preferred formats and cadences.
